...
public class Utils {
	...
	public static Vector3 PutOnGround(Vector2 v, float defaultZ = 0f) {
		return PutOnGround(Vec2to3(v), ~0, defaultZ);
	}

	public static Vector3 PutOnGround(Vector3 v, float defaultZ = 0f) {
		return PutOnGround(v, ~0, defaultZ);
	}
	
	public static Vector3 PutOnGround(Vector3 v, LayerMask whatIsGround, float defaultZ = 0f) {
		Ray ray = new Ray(new Vector3(v.x, v.y, HIGHEST_GROUND_POINT), Vector3.down);
		RaycastHit hit;
